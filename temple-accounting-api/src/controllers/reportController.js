...
const getLastDayOfMonth = (month) => {
  ...
  const lastDay = new Date(year, monthNum, 0).getDate(); // Day 0 of next month = last day of current month
  return `${year}-${monthNum.padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
};
...
export const getSummary = async (c) => {
  try {
    ...
    // Get summary by category using direct query
    ...
    categoryData.forEach(item => {
      ...
      const categoryType = item.categories.type;
      ...
      categorySummary[categoryName].total += amount;
      categorySummary[categoryName].count += 1;
    });

    const groupedCategoryData = Object.values(categorySummary);
    ...
    return c.json({
      ...
      by_category: groupedCategoryData || []
    });
  } catch (error) {
